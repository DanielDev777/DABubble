...
def test_create_channel_makes_owner_a_member(make_user, client_for):
    ...
    assert "is_private" not in response.data
...
@pytest.mark.django_db
def test_list_shows_only_channels_you_are_in(make_user, client_for):
    owner = make_user("owner@example.com")
    other = make_user("other@example.com")
    client_for(owner).post("/api/channels/", {"name": "Owned"}, format="json")
    names = {c["name"] for c in client_for(other).get("/api/channels/").data}
    assert "Owned" not in names


@pytest.mark.django_db
def test_retrieve_channel_as_non_member_is_404(make_user, client_for):
    owner = make_user("owner@example.com")
    other = make_user("other@example.com")
    created = client_for(owner).post("/api/channels/", {"name": "Owned"}, format="json")
    cid = created.data["id"]
    assert client_for(other).get(f"/api/channels/{cid}/").status_code == 404
